...
impl ConfigService {
    ...
    pub fn get(&self, key: &str) -> Option<String> {
        match key {
            "hotkey"               => Some(self.cache.hotkey.clone()),
    ...
    fn apply_to_cache(&mut self, key: &str, value: &str) {
        match key {
            "hotkey"               => self.cache.hotkey = value.to_string(),
            ...
            "history_limit"        => {
                if let Ok(n) = value.parse::<i64>() { self.cache.history_limit = n; }
            }
...
fn load_config_from_db(conn: &Connection) -> Result<AppConfig, AppError> {
    ...
    for row in rows {
        ...
        match key.as_str() {
            ...
            "hotkey"               => { config.hotkey = ps(&raw).unwrap_or(config.hotkey); }
            "target_lang"          => { config.target_lang = ps(&raw).unwrap_or(config.target_lang); }
            "provider"             => { config.provider = ps(&raw).unwrap_or(config.provider); }
            "auto_start"           => { config.auto_start = raw.trim() == "true"; }
            "history_limit"        => { if let Ok(n) = raw.trim().parse::<i64>() { config.history_limit = n; } }
            "theme"                => { config.theme = ps(&raw).unwrap_or(config.theme); }
            "fallback_enabled"     => { config.fallback_enabled = raw.trim() == "true"; }
            "onboarding_completed" => { config.onboarding_completed = raw.trim() == "true"; }
            _ => {}
        }
    }
    Ok(config)
